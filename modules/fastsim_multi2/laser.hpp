...
namespace fastsim
{
  ...
  public:
    ...
    float update(const Posture& pos,
		 const boost::shared_ptr<Map>& map, float dist);
    ...
    void set_xy_pixel(const Posture& pos, const boost::shared_ptr<Map>& m) 
    {
      ...
      _x_pixel = m->real_to_pixel(x2);
      _y_pixel = m->real_to_pixel(y2);
