  ...
  class IlluminatedSwitch
  {
    public:
      ...
      IlluminatedSwitch(int color, float radius, float x, float y, bool on) :
        ...
        _radius(radius),
        _x(x), _y(y),
        _on(on), _activated(false), _is_robot(false), _solid(false), _display_color(color)
      {
      }

      IlluminatedSwitch(int color, float radius, float x, float y, bool on, unsigned int color_alt) :
        _color(color),
        _radius(radius),
        _x(x), _y(y),
        _on(on), _activated(false), _is_robot(false), _solid(false), _display_color(color_alt)
      {
      }
      ...
      IlluminatedSwitch(int color, float radius, float x, float y, bool on, float bbx, float bby, float bbw, float bbh) :
        ...
        _on(on), _activated(false), _is_robot(true), _solid(true), _display_color(color)
      {
      }
      ...
      unsigned int  get_display_color() const { return _display_color; }
      void set_display_color(unsigned display_color) { _display_color = display_color; }
      ...
      void set_solid(bool solid) { _solid = solid; }
      bool is_solid() const { return _solid; }
